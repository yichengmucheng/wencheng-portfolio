...
const phaseMeta = [
  ...
  ["已获得有效证据", "running", "Mock 事实 · TTL 5m"],
  ...
  return (
    <div className="agent-demo" data-phase={phase}>
      <div className="agent-demo-bar">
        <span><i /><i /><i /></span>
        <b>飞书 · MaaS 百事通 <em>SIMULATION</em></b>
        <button type="button" onClick={reset}>重置</button>
      </div>
      <div className="agent-demo-grid">
        <div className="agent-chat" aria-label="模拟飞书对话">
          ...
          {phase >= 4 && <div className="agent-message"><span className="agent-avatar bot">M</span><div className="agent-approval">
            ...
            <div className="agent-approval-product"><strong>影眸 X3</strong><span>模拟价格 ¥ 12,800 / 套</span></div>
            <dl><div><dt>目标</dt><dd>销售一部 · 12 名模拟成员</dd></div><div><dt>依据</dt><dd>Mock 价管系统 · 10:24:08</dd></div><div><dt>有效期</dt><dd>5 分钟</dd></div></dl>
          ...
          {phase === 5 && <div className="agent-message"><span className="agent-avatar bot">M</span><div><small>MaaS 百事通</small><p className="bot-bubble success"><b>模拟发送完成</b><br />Delivery 回执：<code>om_mock_7F3A</code>。重复确认不会再次发送。</p></div></div>}
          {phase === 0 && <div className="agent-demo-start"><button type="button" onClick={start}>运行模拟任务</button><small>浏览器内状态机 · 不连接真实业务系统</small></div>}
        </div>
